...
#if defined(CUPTI_OPENACC_SUPPORT)
 ...
 * CUpti_ActivityKernel4 activity record.
 */
typedef struct PACKED_ALIGNMENT {
  /**
 ...
 * (CUPTI_ACTIVITY_KIND_KERNEL and
 * CUPTI_ACTIVITY_KIND_CONCURRENT_KERNEL).
 * Kernel activities are now reported using the CUpti_ActivityKernel4 activity
 * record.
 */
typedef struct PACKED_ALIGNMENT {
  /**
 ...
 * CUpti_ActivityDevice2 activity record.
 */
typedef struct PACKED_ALIGNMENT {
  /**
...
typedef struct PACKED_ALIGNMENT {
    ...
    */
    ...
    /**
    * If typeDev0 is CUPTI_DEV_TYPE_GPU, UUID for device 0. \ref CUpti_ActivityDevice2.
    ...
    */
    ...
    /**
    * If typeDev1 is CUPTI_DEV_TYPE_GPU, UUID for device 1. \ref CUpti_ActivityDevice2.
 ...
 * \brief Cleanup CUPTI
 * 
 * Explicitly destroys and cleans up all resources associated with CUPTI 
 * in the current process. Any subsequent CUPTI API call will reinitialize CUPTI.
 * The CUPTI client needs to make sure that required CUDA synchronization and 
 * CUPTI activity buffer flush is done before calling cuptiFinalize.
 */
 ...
 * for kernels whose records are provided through the struct \ref CUpti_ActivityKernel4.
