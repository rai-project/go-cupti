...
static void printUsage() {
  ...
  printf("       -help                                            : displays "
         "help message\n");
  printf("       -device <dev_id> -getdomains                     : displays "
         "supported domains for specified device\n");
  printf("       -device <dev_id> -getmetrics                     : displays "
         "supported metrics for specified device\n");
  printf("       -device <dev_id> -domain <domain_id> -getevents  : displays "
         "supported events for specified domain and device\n");
  printf("Note: default device is 0 and default domain is first domain for "
         "device\n");
}
...
int enumEventDomains(CUdevice dev) {
  ...
  for (i = 0; i < maxDomains; i++) {
    ...
    // query domain name
    size = NAME_SHORT;
    ptiStatus = cuptiEventDomainGetAttribute(
        domainData.domainId, CUPTI_EVENT_DOMAIN_ATTR_NAME, &size,
    ...
    ptiStatus = cuptiDeviceGetEventDomainAttribute(
        dev, domainData.domainId, CUPTI_EVENT_DOMAIN_ATTR_INSTANCE_COUNT, &size,
    ...
    ptiStatus = cuptiDeviceGetEventDomainAttribute(
        dev, domainData.domainId, CUPTI_EVENT_DOMAIN_ATTR_TOTAL_INSTANCE_COUNT,
        &size, (void *)&domainData.totalInstanceCnt);
    CHECK_CUPTI_ERROR(ptiStatus, "cuptiDeviceEventDomainGetAttribute");

    size = sizeof(CUpti_EventCollectionMethod);
    ptiStatus = cuptiEventDomainGetAttribute(
        domainData.domainId, CUPTI_EVENT_DOMAIN_ATTR_COLLECTION_METHOD, &size,
...
int enumEvents(CUpti_EventDomainID domainId) {
  ...
  ptiStatus = cuptiEventDomainGetNumEvents(domainId, &maxEvents);
  ...
  ptiStatus = cuptiEventDomainEnumEvents(domainId, &size, eventId);
  ...
  for (i = 0; i < maxEvents; i++) {
    ...
    size = NAME_SHORT;
    ptiStatus =
        cuptiEventGetAttribute(eventData.Id.eventId, CUPTI_EVENT_ATTR_NAME,
                               &size, (uint8_t *)eventData.eventName);
    ...
    ptiStatus = cuptiEventGetAttribute(eventData.Id.eventId,
                                       ...
                                       &size, (uint8_t *)eventData.shortDesc);
    CHECK_CUPTI_ERROR(ptiStatus, "cuptiEventGetAttribute");
    checkNullTerminator(eventData.shortDesc, size, DESC_SHORT);

    size = DESC_LONG;
    ptiStatus = cuptiEventGetAttribute(eventData.Id.eventId,
                                       CUPTI_EVENT_ATTR_LONG_DESCRIPTION, &size,
    ...
    ptiStatus =
        cuptiEventGetAttribute(eventData.Id.eventId, CUPTI_EVENT_ATTR_CATEGORY,
                               &size, (&eventData.category));
    CHECK_CUPTI_ERROR(ptiStatus, "cuptiEventGetAttribute");
    ...
    printf("\t\"long_description\": \"%s\"", eventData.longDesc);
...
int enumMetrics(CUdevice dev) {
  ...
  for (i = 0; i < maxMetrics; i++) {
    ...
    size = NAME_SHORT;
    ptiStatus =
        cuptiMetricGetAttribute(metricData.Id.metricId, CUPTI_METRIC_ATTR_NAME,
                                &size, (uint8_t *)metricData.eventName);
    CHECK_CUPTI_ERROR(ptiStatus, "cuptiEventGetAttribute");
    ...
    ptiStatus = cuptiMetricGetAttribute(metricData.Id.metricId,
                                        ...
                                        &size, (uint8_t *)metricData.shortDesc);
    CHECK_CUPTI_ERROR(ptiStatus, "cuptiEventGetAttribute");
    ...
    ptiStatus = cuptiMetricGetAttribute(metricData.Id.metricId,
                                        ...
                                        &size, (uint8_t *)metricData.longDesc);
    CHECK_CUPTI_ERROR(ptiStatus, "cuptiEventGetAttribute");
...
void parseCommandLineArgs(int argc, char *argv[], int &deviceId,
